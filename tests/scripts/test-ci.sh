...
check "marketplace.json is valid JSON" "cat '$REPO_ROOT/.claude-plugin/marketplace.json' | node -e 'let d=\"\";process.stdin.on(\"data\",c=>d+=c);process.stdin.on(\"end\",()=>{JSON.parse(d)})'"
